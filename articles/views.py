...
def create(request):
    if request.method == 'POST':
        ...
        # content = request.POST.get('content') (위험) 그대신...       
        if form.is_valid():
...
def detail(request, article_pk): # variable routing 이라 두번재 인자 존재
    article = get_object_or_404(Article, pk=article_pk) # 그 번호에 맞는 article 1개만 꺼내서 저장
